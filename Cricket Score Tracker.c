...
system("cls");
printf("Instructions:-\n");
printf("Enter the score ball by ball by entering the number which represents the result of the delivery.\n");
printf("In illegal deliveries, wicket represents type of dismissals like runouts which happen in extras too.");
printf("\n\nPress any key to Continue");
getch();
system("cls");
...
void first_inning(char y[5]){
    ...
    for(j=0;j!=balls;j++){
label:
printf("Which type of delivery is it?");
printf("\n\n[1]Normal delivery\n[2]No ball/wide\n");
scanf("%d",&scorer);
...
if(wickets==10||p_balls==balls||score>=target){
    ...
    else if(c_overs==overs && score==target-1){
        printf("Match tied");
        exit(0);
    }
    else{
